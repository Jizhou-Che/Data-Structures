...
struct doubly_linked_int_list{
	int value;
	struct doubly_linked_int_list * prev;
	struct doubly_linked_int_list * next;
};

int list_append(struct doubly_linked_int_list ** head_ref, struct doubly_linked_int_list ** tail_ref, int new_value){
...
int list_detach(struct doubly_linked_int_list ** head_ref, struct doubly_linked_int_list ** tail_ref, int * detached_value_ref){
	if(*tail_ref == NULL){
		...
		*detached_value_ref = (*tail_ref)->value;
		struct doubly_linked_int_list * temp = *tail_ref;
...
int list_pop(struct doubly_linked_int_list ** head_ref, struct doubly_linked_int_list ** tail_ref, int * popped_value_ref){
	if(*head_ref == NULL){
		...
		*popped_value_ref = (*head_ref)->value;
		struct doubly_linked_int_list * temp = *head_ref;
...
int list_delete(struct doubly_linked_int_list ** head_ref, struct doubly_linked_int_list ** tail_ref, int value_to_del){
	if(*head_ref == NULL){
		...
		if((*head_ref)->value == value_to_del){
			...
			struct doubly_linked_int_list * current_node = (*head_ref)->next;
			while(current_node != NULL){
				if(current_node->value == value_to_del){
					if(current_node->next == NULL){
						*tail_ref = current_node->prev;
						(*tail_ref)->next = NULL;
						free(current_node);
					}else{
...
void list_clear(struct doubly_linked_int_list ** head_ref, struct doubly_linked_int_list ** tail_ref){
	...
	while(current_node != NULL){
		struct doubly_linked_int_list * temp = current_node;
		current_node = current_node->next;
		free(temp);
	}
	*head_ref = NULL;
...
int main(){
	...
	struct doubly_linked_int_list * tail_of_list = NULL;
	...
	while(1){
		...
		if(option == 1){
			...
			struct doubly_linked_int_list * previous_of_new = NULL;
			int value_to_insert;
